...
pacman::p_load(data.table, matrixStats)
...
meds<-as.data.table(cbind(counts$GC_content, counts$med))
...
#write.table(REF, "../results/pca/gc_bins.txt", sep="\t", row.names=T, quote=F)
...
fwrite(proportions, file=args[3], sep="\t", quote=F)
